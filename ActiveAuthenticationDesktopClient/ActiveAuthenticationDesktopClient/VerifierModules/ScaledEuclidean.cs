...
{
    public class ScaledEuclidean : Verifier
    {
        ...
        public override void Verify(DataSet dsFeatures, DataSet dsFeaturesInliers, DataSet dsFeatureLabelGrouped, DataSet dsTemplate, DataSet dsVerifyResults)
        {
            ...
            reqMatchingPairs = Configuration.scaledEuclideanMinMatchingPairs;
            ...
            try
            {
                ...
                foreach (var g in query)
                {
                    ...
                    foreach (var row in g.resultGroup)
                    {
                        if (row.StDev + 1 > 0)
                        {
                            sumOfDiffSquared += Math.Pow((row.Mean - row.FeatureValue + 1) / (row.StDev + 1), 2.0);
                    ...
                    double score = Math.Sqrt(sumOfDiffSquared);
