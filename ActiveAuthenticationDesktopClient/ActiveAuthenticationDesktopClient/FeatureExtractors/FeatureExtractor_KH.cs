...
{
    class FeatureExtractor_KH : FeatureExtractor
    {
        public FeatureExtractor_KH(FeatureExtractorArguments fea) : base (fea)
        {}
                        ...
                        if (vkCode >= 32 && vkCode <= 127) // 96 codes
                        {
                            ...
                            else
                            {
                                ...
                                if (lastValidPress[charIndex] >= 0)
                                {
                                    ...
                                    string featureLabel = dtCollection.Rows[i]["VkCode"].ToString();

                                    // Calculate keyHold time based on difference from release timestamp and its associated press timestamp
                                    long keyHold = Convert.ToInt64(double.Parse(dtCollection.Rows[i]["AbsoluteTimestamp"].ToString()) - double.Parse(dtCollection.Rows[lastValidPress[charIndex]]["AbsoluteTimestamp"].ToString()));

                                    dtFeatures.Rows.Add(securityId, sampleId, FeatureType, featureLabel, keyHold);
