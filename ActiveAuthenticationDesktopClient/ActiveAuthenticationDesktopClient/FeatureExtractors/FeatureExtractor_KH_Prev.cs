...
{
    class FeatureExtractor_KH_Prev : FeatureExtractor
    {
        public FeatureExtractor_KH_Prev(FeatureExtractorArguments fea) : base (fea)
        {}

        public string FeatureType
        {
            get { return "KeyHoldWithPrevVKCode"; }
        }
        ...
        public override DataTable extract(DataTable KeyboardEvents)
        {
            try
            {
                ...
                if (dtCollection.Rows.Count > 1)
                {
                    ...
                    for (int i = 1; i < dtCollection.Rows.Count; i++)
                    {
                        int vkCode = Convert.ToInt16(dtCollection.Rows[i]["VkCode"]);
                        // Check for key press & that only valid inputs are being used
                        if (vkCode >= 32 && vkCode <= 127) // 96 codes
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    // Calculate KeyHoldWithPrevVKCode time based on difference from release timestamp and its associated press timestamp
                                    long KeyHoldWithPrevVKCode = Convert.ToInt64(double.Parse(dtCollection.Rows[i]["AbsoluteTimestamp"].ToString()) - double.Parse(dtCollection.Rows[lastValidPress[charIndex]]["AbsoluteTimestamp"].ToString()));
                                    string featureLabel = dtCollection.Rows[i]["VkCode"].ToString() + '|' + dtCollection.Rows[i - 1]["VkCode"].ToString();
                                    dtFeatures.Rows.Add(securityId, sampleId, FeatureType, featureLabel, KeyHoldWithPrevVKCode);
