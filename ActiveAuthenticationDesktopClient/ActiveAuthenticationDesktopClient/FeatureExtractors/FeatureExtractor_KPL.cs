...
{
    class FeatureExtractor_KPL : FeatureExtractor
    {
        public FeatureExtractor_KPL(FeatureExtractorArguments fea) : base (fea)
        {}
        ...
        public override DataTable extract(DataTable KeyboardEvents)
        {
            try
            {
                ...
                int[,] KPL_individual_counter = new int[96, 96]; // 96 codes

                // Import each row of the dsCollection table into either the key press (KP) lists, or the key release (KR) lists.
                foreach (DataRow row in dtCollection.Rows)
                {
                    ...
                    if (Convert.ToInt16(row["KeyEvent"]) == 1) // press
                    {
                ...
                // Now KP_index is the number of events + 1
                // Should be just the number of events, not events + 1
                // Iterate through KP events
                for (int i = 0; i < KP_index - 1; i++) // ERROR IN ORIGINAL CODE, OUT OF RANGE EXCEPTION, HAD TO SUBTRACT ONE FROM INDEX COUNTER
                {
                    // The characters for this pair
                    int index1 = KP_char[i];
                    int index2 = KP_char[i + 1];
                    ...
                    if (index1 == index2)
                    {
                        ...
                        // If there are an unequal number of presses to releases (someone held down a key and sent a lot of press events), skip / continue.
                        if (press_count != release_count)
                            continue;
                    }

                    // Key press latency = absolute time of the second press - absolute time of the first press
                    int kpl = (int)(KP_times[i + 1] - KP_times[i]);

                    if (KPL_individual_counter[index1, index2] < 200 && kpl >= 0) // 200 ms maximum digram timing, Similarity_MaxDiffForValidMatch
                    {
                        // Enam:  Scan left and right for non alphas, capture the "word", and append it to the line below for word context.
                        ...
                        dtFeatures.Rows.Add(securityId, sampleId, FeatureType, featureLabel, kpl);

                        // Adds one to the value at the index, still not sure why, possibly to prevent divide by 0 errors
                        KPL_individual_counter[index1, index2]++;
                    }
                }
                return dtFeatures;
            }
